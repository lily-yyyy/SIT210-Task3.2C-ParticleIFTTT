...
bool isThereDifference(bool previousInSunlight, bool newInSunlight) {
    if (previousInSunlight != newInSunlight) {
        if (newInSunlight) {
            Particle.publish("sunlight", "entered"); // publish the event, along with the data that it has entered the sunlight.
        } else {
            Particle.publish("sunlight", "left"); // publish the event, along with the data that it has left the sunlight.
        }
    }
